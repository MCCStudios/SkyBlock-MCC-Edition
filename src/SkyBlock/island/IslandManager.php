...
class IslandManager {
    ...
    public function createIsland(Player $owner, $generator) {
        ...
        $config = new Config(Utils::getIslandPath($name."skyblock"), Config::JSON, [
            "owner" => $name,
            "members" => [],
            "locked" => false,
            "home" => "",
        ...
        $this->addIsland($config, $name, $name."skyblock", [], false, "", $generator);
        $config = $this->plugin->getSkyBlockManager()->getPlayerConfig($owner);
        $config->set("island", $name."skyblock");
        $config->save();
    }
...
$worldname = $island->getIdentifier();
...
$levelpath = $server->getDataPath () . "worlds/" . $worldname ."/";
