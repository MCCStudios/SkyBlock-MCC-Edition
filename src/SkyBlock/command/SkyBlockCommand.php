...
class SkyBlockCommand extends Command {
    ...
    public function __construct(SkyBlock $plugin) {
        $this->plugin = $plugin;
        parent::__construct("skyblock", "Main SkyBlock command", "Usage: /skyblock");
    }

    public function sendMessage(Player $sender, $message) {
        $sender->sendMessage(TextFormat::GOLD . "- " . TextFormat::AQUA . $message);
    }
                ...
                switch($args[0]) {
                    case "join":
                        ...
                        if(empty($config->get("island"))) {
                            $this->sendMessage($sender, "You dont have an island yet!");
                        }
                        else {
                            ...
                            if($island instanceof Island) {
                                ...
                                $this->sendMessage($sender, "You dont have an island!");
                            }
                        }
                        break;
                    case "create":
                        ...
                        if(empty($config->get("island"))) {
                            ...
                            $this->sendMessage($sender, "You already have a skyblock island!");
                        }
                        break;
                    case "home":
                        $config = $this->plugin->getSkyBlockManager()->getPlayerConfig($sender);
                        if(empty($config->get("island"))) {
                            $this->sendMessage($sender, "You haven't got a island!");
                        }
                        else {
                            $island = $this->plugin->getIslandManager()->getOnlineIsland($config->get("island"));
                            if($island instanceof Island) {
                                ...
                                $this->sendMessage($sender, "You haven't got an island!!");
                            }
                        }
                        break;
                    case "sethome":
...
case "delete":
                    ...
                    case "help":
                        $commands = [
                            ...
                            "disband" => "delete your island",
"reset" => "restart your island",

                        ];
                        foreach($commands as $command => $description) {
                            $sender->sendMessage(TextFormat::GOLD . "/skyblock {$command}: " . TextFormat::AQUA . $description);
                        }
                        break;
						                    case "info":
                        $commands = [
